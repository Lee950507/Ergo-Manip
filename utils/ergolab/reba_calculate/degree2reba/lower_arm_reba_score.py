class LAREBA:
    ...
    def lower_arm_score_continous(self):
        degree = self.LA_degrees
        right_degree = degree[0]
        left_degree = degree[1]
        right_lower_arm_reba_score = 0
        right_lower_arm_reba_score += 1 + abs(right_degree - 60) / 30
        # if 0 <= right_degree < 60:
        #     right_lower_arm_reba_score += 2 + abs(right_degree - 60) / 120
        # if 60 <= right_degree < 100:
        #     right_lower_arm_reba_score += 1 + abs(right_degree - 80) / 20
        # if 100 <= right_degree:
        #     right_lower_arm_reba_score += 2 + (right_degree - 100) / 120

        left_lower_arm_reba_score = 0
        left_lower_arm_reba_score += 1 + abs(left_degree - 60) / 30
        # if 0 <= left_degree < 60:
        #     left_lower_arm_reba_score += 2 + abs(left_degree - 60) / 120
        # if 60 <= left_degree < 100:
        #     left_lower_arm_reba_score += 1 + abs(left_degree - 80) / 20
        # if 100 <= left_degree:
        #     left_lower_arm_reba_score += 2 + (left_degree - 100) / 120

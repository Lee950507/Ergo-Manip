class UAREBA:
    ...
    def upper_arm_reba_score_continous(self):
        right_upper_arm_reba_score = 0
        right_upper_arm_flex_score = 0
        right_upper_arm_side_score = 0
        right_upper_arm_shoulder_rise = 0
        ...
        if -20 <= right_flexion < 20:
            right_upper_arm_reba_score += 1 + abs(right_flexion) / 20
            right_upper_arm_flex_score += 1 + abs(right_flexion) / 20
        if 20 <= right_flexion < 45:
            right_upper_arm_reba_score += 2 + (right_flexion - 20) / 35
            right_upper_arm_flex_score += 2 + (right_flexion - 20) / 35
        if right_flexion < -20:
            right_upper_arm_reba_score += 2
            right_upper_arm_flex_score += 2
        if 45 <= right_flexion < 90:
            right_upper_arm_reba_score += 3 + (right_flexion - 45) / 45
            right_upper_arm_flex_score += 3 + (right_flexion - 45) / 45
        if 90 <= right_flexion:
            right_upper_arm_reba_score += 4 + (right_flexion - 90) / 90
            right_upper_arm_flex_score += 4 + (right_flexion - 90) / 90

        if -20 <= left_flexion < 20:
            left_upper_arm_reba_score += 1 + abs(left_flexion) / 20
            left_upper_arm_flex_score += 1 + abs(left_flexion) / 20
        if 20 <= left_flexion < 45:
            left_upper_arm_reba_score += 2 + (left_flexion - 20) / 35
            left_upper_arm_flex_score += 2 + (left_flexion - 20) / 35
        if left_flexion < -20:
            left_upper_arm_reba_score += 2
            left_upper_arm_flex_score += 2
        if 45 <= left_flexion < 90:
            left_upper_arm_reba_score += 3 + (left_flexion - 45) / 45
            left_upper_arm_flex_score += 3 + (left_flexion - 45) / 45
        if 90 <= left_flexion:
            left_upper_arm_reba_score += 4 + (left_flexion - 90) / 90
            left_upper_arm_flex_score += 4 + (left_flexion - 90) / 90

            # for side bending
        if abs(right_side) > 2:
            right_upper_arm_reba_score += 1 + (right_side - 2) / 90
            right_upper_arm_side_score += 1 + (right_side - 2) / 90

        if abs(left_side) > 2:
            left_upper_arm_reba_score += 1 + (left_side - 2) / 90
            left_upper_arm_side_score += 1 + (left_side - 2) / 90

            # for shoulder rise

        if right_shoulder_rise > 90:
            right_upper_arm_reba_score += 1 + (right_shoulder_rise - 90) / 120
            right_upper_arm_shoulder_rise += 1 + (right_shoulder_rise - 90) / 120

        if left_shoulder_rise > 90:
            left_upper_arm_reba_score += 1 + (left_shoulder_rise - 90) / 120
            left_upper_arm_shoulder_rise += 1 + (left_shoulder_rise - 90) / 120

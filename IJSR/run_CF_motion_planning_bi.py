...
if __name__ == '__main__':
    ...
    ## Standby
    robot_left_position_standby = np.array([0.85, 0.0, 1.2])
    robot_right_position_standby = np.array([0.85, -0.6, 1.2])
    ...
    robot_left_pose_matrix_standby = np.r_[
        np.c_[robot_left_rotation_matrix_init, robot_left_position_standby.T], np.array([[0, 0, 0, 1]])]
    robot_right_pose_matrix_standby = np.r_[
        np.c_[robot_right_rotation_matrix_init, robot_right_position_standby.T], np.array([[0, 0, 0, 1]])]

    standby_robot_left_pose_matrix = base2torso_matrix @ robot_left_pose_matrix_standby
    standby_robot_right_pose_matrix = base2torso_matrix @ robot_right_pose_matrix_standby

    curi.set_tcp_moveL(standby_robot_left_pose_matrix, standby_robot_right_pose_matrix)

    ## Grasp
    robot_left_position_init = np.array([0.9, -0.15, 1.2])
    robot_right_position_init = np.array([0.9, -0.45, 1.2])
    ...
    if PLANNING_METHOD == 0:
        ...
        w_goal, w_ref, w_ergo = 0.35, 0.65, 0.0
    else:
    ...
    max_displacement_delta_per_step = 0.002 # 4mm per axis per step
    ...
    recorded_timestamps = []
    optimized_robot_positions_left = []
    optimized_robot_positions_right = []
    planned_motion_directions = []  # 每步规划的 motion direction (unit vector), 保存为 npy

    setup_subscribers()

    folder = '/home/clover/Chenzui/Ergo-Manip/data/composite_field/0205/chenzui/8'
    ...
    for step in range(max_plan_steps):
        ...
        if goal_dist < goal_threshold:
            ...
            optimized_robot_positions_left.append(endpoint.copy())
            optimized_robot_positions_right.append(endpoint.copy())
            break
        ...
        optimized_robot_positions_left.append(robot_left_position.copy())
        optimized_robot_positions_right.append(robot_right_position.copy())
    ...
    np.save(os.path.join(folder, 'optimized_robot_positions_left.npy'), np.array(optimized_robot_positions_left))
    np.save(os.path.join(folder, 'optimized_robot_positions_right.npy'), np.array(optimized_robot_positions_right))

...
class PlannerConfig:
    ...
    # tangent guidance
    beta_tan_max: float = 1.0
    d_tan_ref: float = 0.4
    p_tan: float = 2.0

    # progress booster near trajectory
    k_goal: float = 0.5
    goal_decay_width: float = 8.0
    d_goal_ref: float = 0.35

    s_tol_goal: float = 0.02
...
def plan_once(q_start, cfg: PlannerConfig):
    ...
    step = cfg.step_init
    s_goal = cfg.csdf.num_segments - 1
    ...
    for it in range(cfg.max_iters):
        ...
        # 切向 + 进度推进（仅靠近轨迹时启用推进器）
        _, d_demo, u_tan, info = nearest_point_and_tangent_on_demo(cfg.csdf, p)
        s = info["segment"] + info["t"]
        ds = s_goal - s
        beta = cfg.beta_tan_max / (1.0 + (max(d_demo,0.0) / max(1e-6, cfg.d_tan_ref)) ** cfg.p_tan)
        gamma = cfg.k_goal * sigmoid(ds / max(cfg.goal_decay_width, 1e-6)) if d_demo < cfg.d_goal_ref else 0.0

        d = d_grad + (beta + gamma) * u_tan
        ...
        # 终点优先
        if abs(ds) < cfg.s_tol_goal:
            print(f"[iter {it}] Reached goal progress: s={s:.3f}/{s_goal}, d={d_demo:.3f}")
            break

        # 收敛（需有足够历史）
        dp_ok, dc_ok = False, False
        if len(Ps) >= 2:
            dp = np.linalg.norm(Ps[-1] - Ps[-2])
            dp_ok = dp < cfg.tol_step
        if len(costs) >= 2:
            dc = abs(costs[-1] - costs[-2])
            dc_ok = dc < cfg.tol_cost
        if dp_ok or dc_ok:
            msg = f"[iter {it}] Converged:"
            if dp_ok: msg += f" dp={dp:.3e}"
            if dc_ok: msg += f" dc={dc:.3e}"
            msg += f", s={s:.3f}"
            print(msg)
            break
...
def plan_sdf_only(q_start, cfg: PlannerConfig):
    # 初值
    q = wrap_to_pi(np.array(q_start, dtype=float))
    x0, y0 = forward_kinematics(q[0], q[1], cfg.l1, cfg.l2)
    p = np.array([x0, y0], dtype=float)
    ...
    # 用当前 q 的窗口生成一次 SEF，只取掩膜 mask（用于限制 SDF-only 的有效域）
    SEF, _, mask = build_sef_and_composite_at_q(cfg.xs, cfg.ys, cfg.SDF, q, cfg)
    _, c = numeric_gradient(cfg.xs, cfg.ys, cfg.SDF, p, eps=cfg.grad_eps)
    costs.append(c if np.isfinite(c) else np.nan)
    ...
    for it in range(cfg.max_iters):
        g, c = numeric_gradient(cfg.xs, cfg.ys, cfg.SDF, p, eps=cfg.grad_eps)
        if not np.isfinite(c):
            print(f"[SDF-only iter {it}] SDF undefined at p={p}. Stop.")
            break
        ngrad = np.linalg.norm(g)
        if ngrad < 1e-12:
            print(f"[SDF-only iter {it}] grad ~ 0. Stop.")
            break
        d_grad = -g / ngrad
        ...
        for _ in range(18):
            ...
            if not np.isfinite(mask_val) or mask_val < 0.5:
                step_try *= cfg.ls_shrink
                continue
            c_try = bilinear_sample(cfg.xs, cfg.ys, cfg.SDF, p_try)
            if not np.isfinite(c_try) or c_try > c - 1e-6:
                step_try *= cfg.ls_shrink
                continue
            success = True
            break
        if not success:
            print(f"[SDF-only iter {it}] line search failed. Stop.")
            break

        # 受限 IK（与 composite 相同）
        q_next = pick_ik_near(q, p_try[0], p_try[1], cfg)
        if q_next is None:
            print(f"[SDF-only iter {it}] IK near previous failed. Stop.")
            break
        ...
        # 终点优先
        if abs(ds) < cfg.s_tol_goal:
            print(f"[SDF-only iter {it}] Reached goal progress: s={s:.3f}/{s_goal}, d={d_demo:.3f}")
            break

        # 数值收敛（保护）
        dp_ok, dc_ok = False, False
        if len(Ps) >= 2:
            dp = np.linalg.norm(Ps[-1] - Ps[-2]); dp_ok = dp < cfg.tol_step
        if len(costs) >= 2:
            dc = abs(costs[-1] - costs[-2]); dc_ok = dc < cfg.tol_cost
        if dp_ok or dc_ok:
            msg = f"[SDF-only iter {it}] Converged:"
            if dp_ok: msg += f" dp={dp:.3e}"
            if dc_ok: msg += f" dc={dc:.3e}"
            msg += f", s={s:.3f}"
            print(msg)
        ...
        # 动态更新 SEF 掩膜（仅为限制域，SDF 不变）
        SEF, _, mask = build_sef_and_composite_at_q(cfg.xs, cfg.ys, cfg.SDF, q, cfg)

    return np.vstack(Qs), np.vstack(Ps), np.array(costs)


# -------------------------- Joint CSEF (your definition) and plot --------------------------
...
def compute_csef_joint_grid(q1_grid, q2_grid, q_opt, weights, comfort_threshold):
    Q1, Q2 = np.meshgrid(q1_grid, q2_grid, indexing='xy')
    CSEF = calculate_sef_single_optimal(Q1, Q2, float(q_opt[0]), float(q_opt[1]),
                                        comfort_threshold=float(comfort_threshold),
                                        weights=list(weights))
    return Q1, Q2, CSEF

def plot_csef_with_joint_path(Q1, Q2, CSEF, Qs, q_opt, title="CSEF (joint) with joint path", save_png="SEF_2D.png"):
    # 按你风格绘制
    plt.rcParams.update({'font.size': 18})
    cmap = plt.cm.RdBu_r
    vmax = np.nanmax(np.abs(CSEF)); vmin = -vmax
    fig, ax = plt.subplots(figsize=(12, 12))
    levels = np.linspace(vmin, vmax, 21)
    cf = ax.contourf(Q1, Q2, CSEF, levels=levels, cmap=cmap)
    cbar = fig.colorbar(cf, ax=ax); cbar.set_label('CSEF value', fontsize=18)

    contour_levels = np.linspace(vmin, vmax, 11)
    cs = ax.contour(Q1, Q2, CSEF, levels=contour_levels, colors='black', linewidths=0.8)
    ax.clabel(cs, inline=True, fontsize=28, fmt='%.1f')

    zc = ax.contour(Q1, Q2, CSEF, levels=[0.0], colors='green', linewidths=3)
    ax.clabel(zc, inline=True, fontsize=28, fmt='%.1f')

    # 最优点
    ax.plot(float(q_opt[0]), float(q_opt[1]), 'go', markersize=12, label='q_opt')

    # 叠加关节路径
    ax.plot(Qs[:,0], Qs[:,1], '-o', lw=2.2, ms=3.5, color='tab:blue', label='composite joint path')
    ax.scatter(Qs[0,0], Qs[0,1], c='red', s=55, edgecolor='k', zorder=5, label='start')

    ax.set_xlabel('q1', fontsize=30); ax.set_ylabel('q2', fontsize=30)
    ax.tick_params(axis='x', labelsize=30); ax.tick_params(axis='y', labelsize=30)
    ax.set_title(title, fontsize=18)
    ax.legend(loc='best')

    fig.tight_layout()
    if save_png:
        fig.savefig(save_png, dpi=800)
    plt.show()
...
def main():
    ...
    cfg = PlannerConfig(
        ...
        # 推进器
        k_goal=0.5, goal_decay_width=8.0, d_goal_ref=0.35,
        s_tol_goal=0.02,
        # 步进与收敛
    ...
    # 1) 复合场规划
    Qs_comp, Ps_comp, costs_comp, SEF_final, Composite_final, mask_final = plan_once(q_start, cfg)

    # 2) SDF-only 规划（用于对比）
    Qs_sdf, Ps_sdf, costs_sdf = plan_sdf_only(q_start, cfg)

    # 可视化：SEF/Composite（final）与两条路径
    ...
    try:
        ...
        pass
    ax.plot(Ps_comp[:,0], Ps_comp[:,1], color='c', lw=2.5, marker='o', markersize=3.5, label='composite path')
    ax.plot(Ps_sdf[:,0], Ps_sdf[:,1], color='k', lw=2.0, linestyle='--', marker='.', markersize=3.5, label='SDF-only path')
    ax.scatter(Ps_comp[0,0], Ps_comp[0,1], c='red', s=50, edgecolor='k', zorder=5, label='start')
    ...
    try:
        ...
        pass
    ax.plot(Ps_comp[:,0], Ps_comp[:,1], color='c', lw=2.5, marker='o', markersize=3.5, label='composite path')
    ax.plot(Ps_sdf[:,0], Ps_sdf[:,1], color='w', lw=2.0, linestyle='--', marker='.', markersize=3.5, label='SDF-only path')
    ax.scatter(Ps_comp[0,0], Ps_comp[0,1], c='red', s=50, edgecolor='k', zorder=5)
    ...
    # 3) 关节空间 CSEF 场 + 复合场关节轨迹叠加
    q1_grid = np.linspace(-np.pi, np.pi, 181)
    q2_grid = np.linspace(-np.pi, np.pi, 181)
    Q1, Q2, CSEF_joint = compute_csef_joint_grid(q1_grid, q2_grid, cfg.q_opt, cfg.weights_sef, cfg.comfort_threshold)
    plot_csef_with_joint_path(Q1, Q2, CSEF_joint, Qs_comp, cfg.q_opt,
                              title='CSEF (joint) with composite joint path',
                              save_png='SEF_2D.png')

    # 输出终点
    print("Composite final q:", np.round(Qs_comp[-1], 4))
    print("Composite final p:", np.round(Ps_comp[-1], 4))
    print("Composite final cost:", costs_comp[-1])
    print("SDF-only final q:", np.round(Qs_sdf[-1], 4))
    print("SDF-only final p:", np.round(Ps_sdf[-1], 4))
    print("SDF-only final cost:", costs_sdf[-1])

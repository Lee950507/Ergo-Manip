...
box_label = 1
ergo_flag = 1
folder = f'/home/clover/Chenzui/Ergo-Manip/data/box_carrying/0921/wuxi/{box_label}/{ergo_flag}.1'
...
def generate_smooth_trajectory(waypoints, speed_limit, t_total, t_sample, desired_length=None):
    """
    基于给定的路径点生成平滑轨迹
    waypoints: 路径点 [N, 3]
    speed_limit: 最大速度限制
    t_total: 总时间
    t_sample: 采样时间间隔
    desired_length: 期望的轨迹长度(点数)，如果指定，将精确生成该长度的轨迹
    """
    ...
    if desired_length is not None:
        # 如果指定了期望长度，直接计算等间隔时间点
        t_samples = np.linspace(0, t_total, desired_length)
    else:
        # 否则按原有方式采样
    ...
    if max_speed > speed_limit and max_speed > 0:
        ...
        if desired_length is not None:
            # 使用固定长度
            t_samples_adjusted = np.linspace(0, t_total_adjusted, desired_length)
        else:
            # 使用固定时间间隔
...
def find_global_optimal_configuration(
        ...
        joint_angle_bounds, num_samples=500, search_radius=0.5, fine_tuning_steps=50,
        symmetry_weight=0.5):  # 新增权重参数控制对称性重要程度
    """
    ...
    # 计算初始构型相似度 (角度差异的均方根)
    initial_symmetry_diff = np.sqrt(np.mean(np.square(initial_q_l - initial_q_r)))
    best_symmetry_diff = initial_symmetry_diff

    # 计算复合得分 (CSEF和对称性的加权组合)
    def calculate_composite_score(csef_l, csef_r, q_l, q_r):
        max_csef = max(csef_l, csef_r)
        symmetry_diff = np.sqrt(np.mean(np.square(q_l - q_r)))
        return (1 - symmetry_weight) * max_csef + symmetry_weight * symmetry_diff

    initial_score = calculate_composite_score(initial_csef_l, initial_csef_r, initial_q_l, initial_q_r)
    best_score = initial_score

    print(f"初始状态: 左臂CSEF={initial_csef_l:.4f}, 右臂CSEF={initial_csef_r:.4f}, 最大值={best_max_csef:.4f}")
    print(f"初始构型相似度差异: {initial_symmetry_diff:.4f}, 复合得分: {initial_score:.4f}")
    ...
    for i in range(num_samples):
        if i % 100 == 0:
            ...
            print(
                f"当前最佳: 复合得分={best_score:.4f}, 最大CSEF={best_max_csef:.4f}, 构型差异={best_symmetry_diff:.4f}")
        ...
        if abs(current_distance - initial_distance) < 0.05:
            ...
            # 计算构型相似度
            symmetry_diff = np.sqrt(np.mean(np.square(candidate_q_l - candidate_q_r)))

            # 计算复合得分
            composite_score = calculate_composite_score(csef_l, csef_r, candidate_q_l, candidate_q_r)
            ...
            if composite_score < best_score:
                ...
                best_symmetry_diff = symmetry_diff
                best_score = composite_score
                print(f"找到更好的解: 复合得分={best_score:.4f}, 最大CSEF={max_csef:.4f}, 构型差异={symmetry_diff:.4f}")
        ...
        for _ in range(50):  # 每步细化进行50次尝试
            ...
            if abs(current_distance - initial_distance) < 0.02:
                ...
                if composite_score < best_score:
                    ...
                    best_score = composite_score
                    print(
                        f"细化阶段找到更好的解: 复合得分={best_score:.4f}, 最大CSEF={max_csef:.4f}, 构型差异={symmetry_diff:.4f}")

    # 计算最终状态的CSEF值
    final_csef_l = calculate_sef(best_q_l, comfort_threshold)
    final_csef_r = calculate_sef(best_q_r, comfort_threshold)
    final_symmetry_diff = np.sqrt(np.mean(np.square(best_q_l - best_q_r)))
    final_score = calculate_composite_score(final_csef_l, final_csef_r, best_q_l, best_q_r)
    ...
    print(f"初始构型相似度差异: {initial_symmetry_diff:.4f}")
    print(f"初始复合得分: {initial_score:.4f}")
    print("\n")
    print(
        f"优化后: 左臂CSEF={final_csef_l:.4f}, 右臂CSEF={final_csef_r:.4f}, 最大值={max(final_csef_l, final_csef_r):.4f}")
    print(f"优化后构型相似度差异: {final_symmetry_diff:.4f}")
    print(f"优化后复合得分: {final_score:.4f}")
    print("\n")
    print(f"CSEF最大值改善: {max(initial_csef_l, initial_csef_r) - max(final_csef_l, final_csef_r):.4f}")
    print(f"构型相似度改善: {initial_symmetry_diff - final_symmetry_diff:.4f}")
    print(f"复合得分改善: {initial_score - final_score:.4f}")
    print(
        f"距离约束: 初始={initial_distance:.4f}m, 最终={final_distance:.4f}m, 误差={abs(final_distance - initial_distance):.4f}m")

    # 详细展示左右臂关节角对比
    print("\n左右臂关节角对比:")
    for i in range(len(best_q_l)):
        print(
            f"关节 {i + 1}: 左臂={best_q_l[i]:.4f}, 右臂={best_q_r[i]:.4f}, 差异={abs(best_q_l[i] - best_q_r[i]):.4f}")
...
def run_iterations(num_iterations):
    ...
    plt.savefig(os.path.join(folder, 'final_csef.png'))
    plt.show()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(folder, 'optimal_csef.png'))
    plt.show()
...
latest_shouL_msg = None
latest_elbowL_msg = None
latest_wristL_msg = None
...
def shouL_callback(msg):
    global latest_shouL_msg
    latest_shouL_msg = msg


def elbowL_callback(msg):
    global latest_elbowL_msg
    latest_elbowL_msg = msg


def wristL_callback(msg):
    global latest_wristL_msg
    latest_wristL_msg = msg
...
def setup_subscribers():
    ...
    rospy.Subscriber('/vrpn_client_node/shouL/pose', PoseStamped, shouL_callback)
    rospy.Subscriber('/vrpn_client_node/elbowL/pose', PoseStamped, elbowL_callback)
    rospy.Subscriber('/vrpn_client_node/wristL/pose', PoseStamped, wristL_callback)
...
if __name__ == '__main__':
    ...
    ## Grasp
    robot_left_position_grasp = np.array([0.8, 0.25, 0.85])
    robot_right_position_grasp = np.array([0.8, -0.25, 0.85])
    ...
    robot_left_pose_matrix_grasp = np.r_[
        np.c_[robot_left_rotation_matrix_init, robot_left_position_grasp.T], np.array([[0, 0, 0, 1]])]
    robot_right_pose_matrix_grasp = np.r_[
        np.c_[robot_right_rotation_matrix_init, robot_right_position_grasp.T], np.array([[0, 0, 0, 1]])]

    grasp_robot_left_pose_matrix = base2torso_matrix @ robot_left_pose_matrix_grasp
    grasp_robot_right_pose_matrix = base2torso_matrix @ robot_right_pose_matrix_grasp

    curi.set_tcp_moveL(grasp_robot_left_pose_matrix, grasp_robot_right_pose_matrix)

    ## Initialization of robot end effector poses
    robot_left_position_init = np.array([0.8, 0.25, 0.95])
    robot_right_position_init = np.array([0.8, -0.25, 0.95])
    ...
    global_positions[3] = shouR_position_init
    global_positions[6] = shouL_position_init
    ...
    print("平滑处理轨迹...")
    smoothed_trajectory_hand_l = smooth_trajectory(np.array(trajectory_hand_l), smoothing_factor=0.8, iterations=5)
    smoothed_trajectory_hand_r = smooth_trajectory(np.array(trajectory_hand_r), smoothing_factor=0.8, iterations=5)

    waypoints_ergo_l = smoothed_trajectory_hand_l
    waypoints_straight_l = np.array([smoothed_trajectory_hand_l[0], smoothed_trajectory_hand_l[-1]])

    # 生成轨迹 - 右臂
    waypoints_ergo_r = smoothed_trajectory_hand_r
    waypoints_straight_r = np.array([smoothed_trajectory_hand_r[0], smoothed_trajectory_hand_r[-1]])
    ...
    print("生成平滑轨迹...")
    if ergo_flag == 1:
        trajectory_ergo_l = generate_smooth_trajectory(waypoints_ergo_l, speed_limit, t_total, t_sample)
        trajectory_ergo_r = generate_smooth_trajectory(waypoints_ergo_r, speed_limit, t_total, t_sample,
                                                       desired_length=len(trajectory_ergo_l))
    else:
        trajectory_ergo_l = generate_smooth_trajectory(waypoints_straight_l, speed_limit, t_total, t_sample)
        trajectory_ergo_r = generate_smooth_trajectory(waypoints_straight_r, speed_limit, t_total, t_sample,
                                                       desired_length=len(trajectory_ergo_l))

    print(f"左臂轨迹长度: {len(trajectory_ergo_l)}, 右臂轨迹长度: {len(trajectory_ergo_r)}")
    ...
    plt.subplot(1, 3, 2)
    plt.plot(trajectory_ergo_l[:, 1], label='Left Arm Y')
    plt.plot(trajectory_ergo_r[:, 1], label='Right Arm Y')
    plt.title('Y Position')
    plt.grid(True)

    plt.subplot(1, 3, 3)
    plt.plot(trajectory_ergo_l[:, 2], label='Left Arm Z')
    plt.plot(trajectory_ergo_r[:, 2], label='Right Arm Z')
    plt.title('Z Position')
    plt.grid(True)

    plt.tight_layout()
    plt.savefig(os.path.join(folder, 'trajectory_ergo.png'))
    plt.show()


    # 标准化位置
    position_l = trajectory_ergo_l[:] - trajectory_ergo_l[0]
    position_r = trajectory_ergo_r[:] - trajectory_ergo_r[0]
    ...
    while (latest_shouR_msg is None or latest_elbowR_msg is None or latest_wristR_msg is None or latest_shouL_msg is None or latest_elbowL_msg is None or latest_wristL_msg is None) and \
            (rospy.Time.now() - start_time).to_sec() < 3.0:  # 5秒超时
        time.sleep(0.1)

    if latest_shouR_msg is None or latest_elbowR_msg is None or latest_wristR_msg is None or latest_shouL_msg is None or latest_elbowL_msg is None or latest_wristL_msg is None:
        rospy.logwarn("Not all initial pose messages received. Proceeding anyway.")

    # 记录位置数据
    recorded_shoulder_positions_l = []
    recorded_elbow_positions_l = []
    recorded_wrist_positions_l = []
    recorded_shoulder_positions_r = []
    recorded_elbow_positions_r = []
    recorded_wrist_positions_r = []
    recorded_timestamps = []

    setup_subscribers()

    start_time = time.time()
    emg_processor = EMGProcessor(channel_num=6, sample_fre=200, start_time=start_time, save=True, save_folder=folder)
    ...
    print(f"开始执行轨迹，共{len(position_l)}个点...")
    for i in range(len(position_l)):
        ...
        # 使用最新可用的消息
        if latest_shouR_msg and latest_elbowR_msg and latest_wristR_msg and latest_shouL_msg and latest_elbowL_msg and latest_wristL_msg:
            # 转换为位置数据
            ...
            sub_shouL = transform_to_pose(latest_shouL_msg)
            sub_elbowL = transform_to_pose(latest_elbowL_msg)
            sub_wristL = transform_to_pose(latest_wristL_msg)
            ...
            shouL_position = T_optitrack2robotbase[:3, :3] @ sub_shouL[:3] + T_optitrack2robotbase[:3, 3]
            elbowL_position = T_optitrack2robotbase[:3, :3] @ sub_elbowL[:3] + T_optitrack2robotbase[:3, 3]
            wristL_position = T_optitrack2robotbase[:3, :3] @ sub_wristL[:3] + T_optitrack2robotbase[:3, 3]

            # 记录位置数据
            recorded_shoulder_positions_r.append(shouR_position.copy())
            recorded_elbow_positions_r.append(elbowR_position.copy())
            recorded_wrist_positions_r.append(wristR_position.copy())
            recorded_shoulder_positions_l.append(shouL_position.copy())
            recorded_elbow_positions_l.append(elbowL_position.copy())
            recorded_wrist_positions_l.append(wristL_position.copy())
            recorded_timestamps.append(current_timestamp.to_sec())


        robot_left_position = robot_left_position_init + position_r[i]
        robot_right_position = robot_right_position_init + position_l[i]
    ...
    recorded_data = {
        ...
        'shoulder_positions': recorded_shoulder_positions_r,
        'elbow_positions': recorded_elbow_positions_r,
        'wrist_positions': recorded_wrist_positions_r,
        'shoulder_positions_l': recorded_shoulder_positions_l,
        'elbow_positions_l': recorded_elbow_positions_l,
        'wrist_positions_l': recorded_wrist_positions_l
    }

    np.save(f'{folder}/recorded_human_position.npy', recorded_data)
    np.save(f'{folder}/optimized_robot_positions_l.npy', position_r)
    np.save(f'{folder}/optimized_robot_positions_r.npy', position_l)
    np.save(f'{folder}/optimized_joint_angles_l.npy', joint_history_l)
    np.save(f'{folder}/optimized_joint_angles_r.npy', joint_history_l)
    np.save(f'{folder}/ergonomics_scores_l.npy', score_history_l)
    np.save(f'{folder}/ergonomics_scores_r.npy', score_history_r)

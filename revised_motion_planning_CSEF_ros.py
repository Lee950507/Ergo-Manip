...
METHOD_NAMES = {
    ...
    METHOD_DUAL_BASED: 'Dual-based',
    METHOD_RESERVED_4: 'Reserved-4',
}
...
def execute_dual_based_follower(curi, t_total, t_sample,
                                robot_left_position_init, robot_right_position_init,
                                robot_left_rotation_matrix_init, robot_right_rotation_matrix_init,
                                base2torso_matrix, T_optitrack2robotbase, start_time,
                                recorded_shoulder_positions, recorded_elbow_positions,
    ...
    """
    Dual-based planning: human moves toward the optimized posture while the robot
    follows human wrist displacement in real time (follower mode).
    """
    print("Dual-based planning: observe the optimized posture, then move toward it.")
    ...
    if wrist_baseline is None:
        ...
        if wrist_baseline_fallback is None:
            raise RuntimeError("No wrist baseline available for dual-based follower.")
    ...
    print(f"Dual-based follower started. Duration={t_total}s, baseline wrist={wrist_baseline.round(4)}")
    while time.time() - exec_start < t_total and not rospy.is_shutdown():
        ...
        if step % 100 == 0:
            print(f"Dual-based step {step}: wrist_delta={wrist_delta.round(4)}, "
...
def execute_method_reserved_4(*args, **kwargs):
    """Placeholder for the fourth planning method."""
    raise NotImplementedError(
        "Planning method 4 is not implemented yet. "
        "Implement execute_method_reserved_4() when the fourth method is ready.")
...
if __name__ == '__main__':
    ...
    if PLANNING_METHOD in (METHOD_CSEF, METHOD_POINT_TO_POINT):
        optimized_robot_positions = execute_open_loop_trajectory(
            position_trajectory=position_trajectory, **common_exec_kwargs)
    elif PLANNING_METHOD == METHOD_DUAL_BASED:
        optimized_robot_positions = execute_dual_based_follower(
            t_total=t_total, t_sample=t_sample,
            wrist_baseline_fallback=wristR_position_init, **common_exec_kwargs)
    elif PLANNING_METHOD == METHOD_RESERVED_4:
        optimized_robot_positions = execute_method_reserved_4(**common_exec_kwargs)
    else:
        raise ValueError("Unknown PLANNING_METHOD: {}. Valid values: 0-3.".format(PLANNING_METHOD))
